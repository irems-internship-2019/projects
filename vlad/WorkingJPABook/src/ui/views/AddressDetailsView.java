...
import ui.filter.enums.AdressEnums;
import ui.filter.labelsprovider.AdressLabelProvider;
...
public class AddressDetailsView extends ViewPart {
	...
	private ArrayList<ContactsTable> listForDetailsView = new ArrayList<ContactsTable>();
	...
	public void setInputForViewer(ContactsTable contact) {
		ArrayList<ContactsTable> contacts = beanRemote.loadContatcs();
	...
	public boolean equalityCheck(long l) {
		boolean check = false;

		for (ContactsTable cnt : listForDetailsView) {

			if (cnt.getContactid() == l)
				check = true;
		}
