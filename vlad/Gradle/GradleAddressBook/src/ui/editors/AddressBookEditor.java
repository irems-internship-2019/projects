...
public class AddressBookEditor extends EditorPart {
	...
	private model.ContactsTable contact;


	public static final String ID = "adress.book.editor";

	public static void openEditor(ContactsTable model) {
	...
	private void setModel(ContactsTable model) {
		// TODO Auto-generated method stub
		this.contact = model;

	}

	public ContactsTable getModel() {
	...
	private void updateWidgets() {
		...
		if (contact != null) {
			...
			emailAdressText.setText(contact.getEmailAddress());
		} else {
	...
	private void editState() {
		...
		if ((contact.getFirstName().equals(firstNameText.getText()) == false
				...
				|| contact.getEmailAddress().equals(emailAdressText.getText()) == false
		...
		) && (regex.isEmailValid(emailAdressText.getText()) && regex.isPhoneNumberValid(phoneNumberText.getText()))) {
			...
			contact.setEmailAddress(emailAdressText.getText());

			contact.getAddress().setCountry(countryText.getText());
			contact.getAddress().setCity(cityText.getText());
			contact.getAddress().setStreet(adressText.getText());
			contact.getAddress().setPostalCode(postalCodeText.getText());

 beanRemote.updateContacts(contact);
...
//			refreshDetailsView();
	...
	private void InsertState() {
		...
		if (isDirty() && regex.isEmailValid(emailAdressText.getText())
				&& regex.isPhoneNumberValid(phoneNumberText.getText())) {

			 contact = new ContactsTable(0, firstNameText.getText(),
					new AddressTable(countryText.getText(), cityText.getText(), adressText.getText(),
							postalCodeText.getText()),
					lastNameText.getText(), phoneNumberText.getText(), emailAdressText.getText());

			beanRemote.insertContact(contact);
	...
	private void refreshDetailsView() {
		...
		IWorkbenchPage activePage = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage();
		AddressContactsView contactsView = (AddressContactsView) activePage.findView(AddressContactsView.ID);
		AddressDetailsView detailsViewview = (AddressDetailsView) activePage.findView(AddressDetailsView.ID);

		detailsViewview.refresh();
